...
const NearbyWasteBank: React.FC = () => {
  ...
  // Simulasi loading daftar bank sampah setelah lokasi diizinkan
  useEffect(() => {
    if (locationAllowed) {
  ...
  const filteredBanks = dummyWasteBanks.filter((item) =>
  ...
  return (
    <div className="max-w-md mx-auto p-4 pb-24">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xl font-semibold">Nearby Waste Bank</h2>
      </div>
      ...
      {!locationAllowed ? (
        <div className="bg-yellow-100 text-sm text-yellow-800 p-3 rounded-lg mb-4">
          <p>
            <strong>Location Permission Required:</strong> Please allow access to your location to show nearby waste banks.
          </p>
        </div>
      ) : (
      ...
      {!locationAllowed ? null : loadingBanks ? (
        ...
        filteredBanks.map((bank) => (
          <div
            key={bank.id}
            onClick={() => navigate(`/WasteBankDetail/${bank.id}`)}
...
export default NearbyWasteBank;
